...
def render_header(
    ...
    with colL:
        if show_home and st.button("← 홈", key=f"hdr_home_{title}"):
            nav_to("홈")
    ...
    with colR:
        ...
        with r1:
            if show_mypage and st.button("마이페이지", key=f"hdr_mypage_{title}"):
                nav_to("마이페이지")
        with r2:
            st.markdown(f"<div style='text-align:right;'>{_badge_html(user_plan)}</div>", unsafe_allow_html=True)
