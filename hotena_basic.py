...
# ============================================================
# ✅ Kanji App (N5~N1) - integrated (single-file)
# - Level buttons (N5~N1)
# - Quiz types: 読み / 意味 / 韓→日
# - 10 questions per page
# ============================================================

KANJI_CSV_PATH = BASE_DIR / "data" / "words_kanji.csv"

def _load_kanji_df():
    """Load kanji dataframe (cached in session)."""
    if "_kanji_df" in st.session_state:
        return st.session_state["_kanji_df"]
    if not KANJI_CSV_PATH.exists():
        st.error(f"한자 CSV를 찾을 수 없습니다: {KANJI_CSV_PATH}")
        st.session_state["_kanji_df"] = None
        return None
    try:
        df = pd.read_csv(KANJI_CSV_PATH, encoding="utf-8-sig")
    except Exception:
        df = pd.read_csv(KANJI_CSV_PATH, encoding="utf-8")
    # normalize
    for c in ["level","pos","jp_word","reading","meaning","example_jp","example_kr"]:
    ...
    st.session_state["_kanji_df"] = df
    return df

def _make_options(values, correct, k=4):
    values = [v for v in values if isinstance(v,str) and v and v != correct]
    random.shuffle(values)
    opts = values[:max(0,k-1)]
    opts.append(correct)
    random.shuffle(opts)
    # ensure length
    while len(opts) < k:
...
def render_kanji_app(sb_authed_local=None, user_email=None, user_id=None, user_plan=None):
    """Render Kanji quiz page inside the integrated app."""
    st.subheader("🈶 한자 퀴즈")
    df = _load_kanji_df()
    if df is None or df.empty:
        st.info("한자 데이터가 없습니다. data/words_kanji.csv 를 확인해 주세요.")
        return

    # state defaults
    if "kanji_level" not in st.session_state:
        st.session_state["kanji_level"] = "N5"
    if "kanji_qtype" not in st.session_state:
        st.session_state["kanji_qtype"] = "reading"  # reading|meaning|kr2jp
    if "kanji_page" not in st.session_state:
        st.session_state["kanji_page"] = 1
    if "kanji_qset" not in st.session_state:
        st.session_state["kanji_qset"] = []
    if "kanji_answers" not in st.session_state:
        st.session_state["kanji_answers"] = {}
    if "kanji_wrong_ids" not in st.session_state:
        st.session_state["kanji_wrong_ids"] = []

    # Level buttons
    cols = st.columns(5)
    for i, lv in enumerate(["N5","N4","N3","N2","N1"]):
        with cols[i]:
            if st.button(lv, key=f"kanji_lv_{lv}", use_container_width=True):
    ...
    st.caption(f"선택 레벨: {st.session_state['kanji_level']}")

    # Quiz type
    qtype_label = st.radio(
        "유형 선택",
        ["발음(読み)", "뜻(意味)", "한→일(韓→日)"],
        horizontal=True,
        index={"reading":0,"meaning":1,"kr2jp":2}.get(st.session_state["kanji_qtype"],0),
        key="kanji_qtype_radio"
    )
    qtype = {"발음(読み)":"reading", "뜻(意味)":"meaning", "한→일(韓→日)":"kr2jp"}[qtype_label]
    if qtype != st.session_state["kanji_qtype"]:
        st.session_state["kanji_qtype"] = qtype
        st.session_state["kanji_page"] = 1
        st.session_state["kanji_qset"] = []
        st.session_state["kanji_answers"] = {}
        st.session_state["kanji_wrong_ids"] = []
    ...
    # Filter by level
    df_lv = df[df["level"] == st.session_state["kanji_level"]].copy()
    if df_lv.empty:
        st.warning("해당 레벨 데이터가 비어 있습니다.")
        return

    # Build / reuse current question set (10)
    QUIZ_LEN = 10
    if not st.session_state["kanji_qset"]:
        # sample unique rows
        sample = df_lv.sample(n=min(QUIZ_LEN, len(df_lv)), replace=False, random_state=None)
        st.session_state["kanji_qset"] = sample.to_dict(orient="records")

    qset = st.session_state["kanji_qset"]
    ...
    with a1:
        if st.button("🔄 새 문제", use_container_width=True):
            ...
            st.rerun()
    with a2:
        if st.button("📝 오답만 다시", use_container_width=True, disabled=(len(st.session_state["kanji_wrong_ids"])==0)):
            wrong_ids=set(st.session_state["kanji_wrong_ids"])
            wrong_rows=[r for r in qset if str(r.get("jp_word","")) in wrong_ids]
            if wrong_rows:
                st.session_state["kanji_qset"] = wrong_rows
                st.session_state["kanji_answers"] = {}
                st.session_state["kanji_wrong_ids"] = []
                st.rerun()
    with a3:
        if st.button("🧹 초기화", use_container_width=True):
            for k in ["kanji_qset","kanji_answers","kanji_wrong_ids"]:
                st.session_state[k] = [] if k!="kanji_answers" else {}
            st.rerun()

    st.divider()

    # Values pool for distractors
    readings = df_lv["reading"].dropna().astype(str).tolist()
    meanings = df_lv["meaning"].dropna().astype(str).tolist()
    jp_words = df_lv["jp_word"].dropna().astype(str).tolist()

    for idx, row in enumerate(qset, start=1):
        jp_word = str(row.get("jp_word","")).strip()
        reading = str(row.get("reading","")).strip()
        meaning = str(row.get("meaning","")).strip()

        st.markdown(f"**Q{idx}.** `{jp_word}`")
        if qtype == "reading":
            correct = reading
            opts = _make_options(readings, correct, 4)
            prompt = "읽기(ひらがな)를 고르세요."
        elif qtype == "meaning":
            correct = meaning
            opts = _make_options(meanings, correct, 4)
            prompt = "뜻을 고르세요."
        else:  # kr2jp
            # show Korean meaning and choose jp_word
            st.write(f"의미: {meaning}")
            correct = jp_word
            opts = _make_options(jp_words, correct, 4)
            prompt = "일본어(한자)를 고르세요."
        ...
        st.session_state["kanji_answers"][idx] = choice
        st.write("")
    ...
    if st.button("✅ 채점", use_container_width=True):
        ...
        for idx, row in enumerate(qset, start=1):
            ...
            reading=str(row.get("reading","")).strip()
            meaning=str(row.get("meaning","")).strip()
            user_choice=st.session_state["kanji_answers"].get(idx)
            if qtype=="reading":
                correct=reading
            elif qtype=="meaning":
                correct=meaning
            ...
            if user_choice == correct:
                ...
                wrong.append(jp_word)
        st.session_state["kanji_wrong_ids"]=wrong
        st.success(f"점수: {score} / {len(qset)}")
        if wrong:
            st.warning("오답: " + ", ".join(wrong))
