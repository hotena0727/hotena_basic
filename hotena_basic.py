...
# ============================================================
# ============================================================
# ✅ Scroll Top Anchor + Helpers (shared core)
# ============================================================
...
def ensure_mastered_words_shape():
    """Ensure session_state['mastered_words'] exists as dict[str, set]."""
    mw = st.session_state.get("mastered_words")
    if not isinstance(mw, dict):
        st.session_state["mastered_words"] = {}
    # also support attribute-style access patterns used in code
    if "mastered_words" not in st.session_state:
        st.session_state.mastered_words = {}

def ensure_excluded_wrong_words_shape():
    """Ensure session_state['excluded_wrong_words'] exists as dict[str, set]."""
    ex = st.session_state.get("excluded_wrong_words")
    if not isinstance(ex, dict):
        st.session_state["excluded_wrong_words"] = {}

def ensure_seen_words_shape():
    """Ensure session_state['seen_words'] exists as dict[str, set]."""
    sw = st.session_state.get("seen_words")
    if not isinstance(sw, dict):
        st.session_state["seen_words"] = {}

def ensure_mastery_banner_shape():
    """Ensure mastery banner flags exist as dict[str,bool]."""
    md = st.session_state.get("mastery_done")
    if not isinstance(md, dict):
        st.session_state["mastery_done"] = {}
    mb = st.session_state.get("mastery_banner_shown")
