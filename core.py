...
def render_top_nav(active: str = "home") -> None:
    """
    ✅ Hotena Top Navigation (Ultra Minimal / Gray)
    - PC/Mobile unified
    - Text-only
    - Active state: thin blue underline
    - Hides Streamlit default chrome (sidebar/menu/footer/header)
    - Preserves rt/at query params
    """
    import streamlit as st
    import textwrap
    ...
    css = textwrap.dedent("""
    <style>
      /* --- Hide Streamlit default UI (safe selectors) --- */
      ...
      /* --- Top nav --- */
      :root{
        --hn-gray-1: rgba(0,0,0,0.82);
        --hn-gray-2: rgba(0,0,0,0.56);
        --hn-line: rgba(0,0,0,0.05);
        --hn-blue: #2f80ed;
      }
      ...
      @media (max-width: 820px){
        .hn-nav{ padding: 10px 10px; gap: 8px; }
        .hn-nav a{ font-size: 13.5px; }
        .hn-nav a.active::after{ left: 34%; width: 32%; }
      }
    </style>
    """)
