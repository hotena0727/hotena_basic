...
# ============================================================
# ✅ Boot rerun: force Streamlit to re-measure layout once
#    - Some environments reserve top chrome height on first paint
#    - A single early rerun often makes the app snap to the correct top position
# ============================================================
def _boot_rerun_once_for_top_gap():
    if st.session_state.get("_boot_rerun_once_for_top_gap_done", False):
        return
    st.session_state["_boot_rerun_once_for_top_gap_done"] = True
    try:
        st.rerun()
    except Exception:
        # older Streamlit
...
<div class="ha-login-hero">
  <div class="ha-login-shell">
    <div class="ha-login-card">
      ...
      <div class="ha-login-note">이제, 일본어로 "말할 수 있는 사람"이 되어보세요.</div>
    </div>
  </div>
</div>
''',
