...
def render_home():
    ...
    # ✅ 트레이닝 홈으로 돌아가기
    if st.button("← 트레이닝 홈", use_container_width=True, key="btn_back_hub"):
...
def render_hub():
    ...
    st.markdown(
        f"""
<div class="jp headbar">
  <div class="headtitle">🏠 하테나 트레이닝 홈</div>
  <div class="headhello">안녕하세요 🙂 <span class="mail">{email}</span></div>
</div>
""",
        unsafe_allow_html=True,
    )
    ...
    st.write("")
    st.caption("원하는 훈련을 선택해 주세요.")

    c1, c2, c3 = st.columns(3)

    with c1:
        st.markdown("### 🧠 단어 훈련")
        st.caption("단어/품사/유형 퀴즈")
        if st.button("단어 훈련 시작", use_container_width=True, key="hub_go_word"):
            st.session_state.page = "home"
            st.rerun()

    with c2:
        st.markdown("### ✍️ 한자 훈련")
        st.caption("쓰기/읽기/복습 (준비중)")
    ...
    with c3:
        st.markdown("### 💬 회화 훈련")
        st.caption("대화/패턴/롤플레이 (준비중)")
        if st.button("회화 훈련", use_container_width=True, key="hub_go_talk"):
            st.session_state.page = "talk"
            st.rerun()

def render_kanji_placeholder():
    st.subheader("✍️ 한자 훈련 (준비중)")
    st.info("여기에 한자 훈련 앱(쓰기/읽기/복습)을 연결하면 됩니다.")
    if st.button("← 트레이닝 홈", use_container_width=True, key="kanji_back_hub"):
        st.session_state.page = "hub"
        st.rerun()

def render_talk_placeholder():
    st.subheader("💬 회화 훈련 (준비중)")
    st.info("여기에 회화 훈련(상황별 대화/패턴/롤플레이)을 연결하면 됩니다.")
    if st.button("← 트레이닝 홈", use_container_width=True, key="talk_back_hub"):
        st.session_state.page = "hub"
        st.rerun()
...
ALLOWED_PAGES = {"hub", "home", "quiz", "my", "admin", "kanji", "talk"}
...
if st.session_state.page == "kanji":
    render_kanji_placeholder()
    st.stop()
