...
# Aliases used in legacy code below (keep names stable)
...
profile = load_profile(sb_authed, user.id)

# ✅ reflect profile into session (plan/admin/name)
plan = (profile.get("plan") or "free").lower()
st.session_state["user_plan"] = plan
st.session_state["is_admin"] = bool(profile.get("is_admin", False))
st.session_state["user_id"] = str(profile.get("id") or user.id)
