            ...
            if row.empty:
                ...
                if st.button("저장", type="primary", key="admin_detail_save", use_container_width=True):
                    try:
                        ...
                        st.caption("선택한 회원의 퀴즈 기록(quiz_attempts) 중 **N일 이전 데이터만** 삭제합니다. (되돌릴 수 없음)")

                        purge_days = st.selectbox("삭제 기준(이전 기록 삭제)", [10, 30, 60, 90, 180, 365], index=1, key="admin_purge_days")
                        scope = st.radio("대상", ["이 회원만", "전체 회원(공통 정책)"], horizontal=True, key="admin_purge_scope")

                        # --- preview / purge helpers ---
                        def _purge_preview_and_delete(do_delete: bool = False):
                            # cutoff (KST 기준으로 보기 좋게, 실제는 ISO로 전달)
                            now_kst = pd.Timestamp.now(tz="Asia/Seoul")
                            cutoff = now_kst - pd.Timedelta(days=int(purge_days))
                            cutoff_iso = cutoff.tz_convert("UTC").isoformat()

                            # target
                            t_user_id = user_id if scope == "이 회원만" else None
                            t_email = (r0.get("email") if isinstance(r0, dict) else None) if scope == "이 회원만" else None

                            def _fetch_rows():
                                # best-effort: user_id 우선, 없으면 user_email
                                q = sb_authed.table("quiz_attempts").select("created_at, user_id, user_email").lt("created_at", cutoff_iso)
                                if t_user_id:
                                    try:
                                        return (q.eq("user_id", t_user_id).limit(5000).execute().data or [])
                                    except Exception:
                                        pass
                                if t_email:
                                    try:
                                        return (q.eq("user_email", t_email).limit(5000).execute().data or [])
                                    except Exception:
                                        pass
                                # 전체 회원
                                try:
                            ...
                            if not do_delete:
                                st.info(f"미리보기: 삭제 대상(최대 5000건 샘플 기준) **{cnt}건** · 기준일: {cutoff.strftime('%Y-%m-%d %H:%M')} (KST)")
                                if cnt == 5000:
                                    st.caption("※ 5000건 이상일 수 있어요(샘플 상한). 그래도 삭제는 조건 기반으로 진행됩니다.")
                                return

                            # delete (best-effort)
                            qd = sb_authed.table("quiz_attempts").delete().lt("created_at", cutoff_iso)
                            if t_user_id:
                                try:
                                    qd = qd.eq("user_id", t_user_id)
                                except Exception:
                                    pass
                            elif t_email:
                                try:
                                    qd = qd.eq("user_email", t_email)
                                except Exception:
                                    pass

                            qd.execute()
                            st.success(f"삭제 완료: {scope} · {purge_days}일 이전 기록 정리")
                            st.rerun()

                        cprev, cdo = st.columns([1, 1], gap="small")
                        with cprev:
                            if st.button("삭제될 건수 미리보기", use_container_width=True, key="admin_purge_preview"):
                                try:
                                    _purge_preview_and_delete(False)
                                except Exception as e:
                                    st.error("미리보기 실패 (RLS/권한/컬럼 확인 필요)")
                                    st.exception(e)

                        agree = st.checkbox("네, 되돌릴 수 없는 삭제임을 이해했습니다.", key="admin_purge_agree")
                        with cdo:
                            if st.button("N일 이전 기록 삭제 실행", type="primary", use_container_width=True, disabled=not agree, key="admin_purge_do"):
                                try:
                                    _purge_preview_and_delete(True)
                                except Exception as e:
                                    st.error("삭제 실패 (RLS/권한/정책 확인 필요)")
                                    st.exception(e)
