...
# - profiles.progress['talk']['mastery'][resume_key] = [qid, ...]
...
def _get_mastered_set(resume_key: str) -> set[str]:
    m = _get_talk_mastery_all()
    lst = m.get(resume_key) or []
    if not isinstance(lst, list):
        lst = []
    return set(str(x) for x in lst if str(x).strip())

def _save_mastered_set(resume_key: str, mastered: set[str]) -> None:
    try:
        m = _get_talk_mastery_all()
        m[resume_key] = sorted(set(str(x) for x in (mastered or set()) if str(x).strip()))
        _set_talk_mastery_all(m)
    except Exception:
        pass
...
def _mark_mastered_once(resume_key: str, qid: str, ok: bool) -> None:
    ...
    try:
        ...
        if st.session_state.get(guard):
            return
        s = _get_mastered_set(resume_key)
        if str(qid) not in s:
            s.add(str(qid))
            # 너무 잦은 저장 방지(해시 가드)
            h = hashlib.md5((resume_key + '|' + '|'.join(sorted(s))).encode('utf-8')).hexdigest()
            if st.session_state.get(f"_talk_mastery_hash__{resume_key}") != h:
                _save_mastered_set(resume_key, s)
...
# ✅ 진도형: 남은 문제가 없으면(전부 완료) 완료 화면 표시 후 종료
if not qids:
    ...
    with st.container(border=True):
        ...
        st.caption("원하시면 진도를 초기화하고 다시 시작할 수 있어요.")
