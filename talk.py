...
# ✅ 하테나쌤 코멘트 (explain_kr 우선, 없으면 hint_kr)
        ...
        with st.expander("🤖 내용이 어려우면 하테나쌤에게 물어보세요", expanded=False):
            ...
            if ask and str(user_q).strip():
                ...
                user_text = str(user_q).strip()
                st.session_state["talk_ai_last_q"] = user_text

                is_admin = bool(st.session_state.get("is_admin", False)) \
                           or bool(st.session_state.get("is_admin_cached", False))

                # ✅ 관리자면 무조건 AI 호출
                if not is_admin and any(k in user_text for k in GENERAL_KEYWORDS):
                    ...
                    with st.spinner("하테나쌤 답변 중…"):
                        ans = ai_tutor.ask_hatena(
                            ...
                            meta={
                                ...
                                "submitted": True,
                                "ok": bool(ok),
                                "is_admin": is_admin,  # ✅ 여기 중요
                                "plan": USER_PLAN,     # ✅ quota 판단용
                            },
                        )
                    coach_slot.info(ans)
# ============================================================
