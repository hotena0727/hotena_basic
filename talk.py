...
"""
    ...
    # --- auto height: prevent clipping on long sentences (Android/iOS) ---
    def _est_lines(s: str, per_line: int) -> int:
        s = (s or "").strip()
        if not s:
            return 0
        # Rough heuristic; good enough to avoid iframe clipping.
        return max(1, int(math.ceil(len(s) / float(per_line))))

    jp_lines = _est_lines(p, 24) + _est_lines(a, 24)
    kr_lines = _est_lines((partner_kr or "").strip(), 34) + _est_lines((answer_kr or "").strip(), 34)
    # Base height assumes 2 JP lines + buttons; add per extra wrapped line.
    _h = 164 + max(0, jp_lines - 2) * 26 + kr_lines * 18
    _h = max(180, min(520, _h))
...
''',
        ...
        # ✅ 보상 지급 규칙:
        # - 반드시 '녹음'을 했고(has_audio)
        # - 점수가 계산되어 있으며(score_key)
        # - 점수가 100점일 때만(정확히 100) 보상을 받을 수 있음
        _score_val = st.session_state.get(score_key)
        _score_val = int(_score_val) if _score_val is not None else None
        reward_eligible = bool(has_audio) and (_score_val is not None) and (_score_val == 100)
        ...
        if st.button("✅ 다 했어요 (보상 받기)", use_container_width=True, key=f"{NS}_next_after"):
            ...
            if reward_eligible:
                ...
                st.warning("보상은 **녹음 후 점수 100점**일 때만 받을 수 있어요. 먼저 녹음하고 점수 계산을 완료해 주세요.")

                # (안전장치) 조건이 풀리면 보상 UI를 숨기고 상태를 초기화
        if st.session_state.get(reward_key) and (not reward_eligible):
            st.session_state[reward_key] = False

if st.session_state.get(reward_key) and reward_eligible:
